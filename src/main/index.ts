...
async function updateAlwaysOnTopByActiveWindow() {
    try {
        ...
        if (active && active.owner) {
            ownerName = String(
                active.owner.name || active.owner.path || "",
            ).toLowerCase();
        }
...
async function createWindow(): Promise<void> {
    ...
    const clampToWorkArea = (
        ...
        try {
            ...
            const clampedX = Math.min(
                Math.max(Math.round(x), minWorkX),
                Math.max(minWorkX, maxWorkX - width),
            );
            const clampedY = Math.min(
                Math.max(Math.round(y), minWorkY),
                Math.max(minWorkY, maxWorkY - height),
            );
        ...
        (
            ...
            if (target && !target.isDestroyed() && width && height) {
                ...
                setTimeout(() => {
                    ...
                    if (windowType === "device")
                        lastDeviceWindowSize = {
                            width: b.width,
                            height: b.height,
                        };
                }, 10);
    ...
    async function openChildWindow(
        ...
        if (typeof sz.x === "number" && typeof sz.y === "number") {
            try {
                win.setPosition(sz.x, sz.y, false);
            } catch {}
        }
