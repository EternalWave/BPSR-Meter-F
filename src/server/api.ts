...
function initializeApi(
        ...
        if (paused) {
            ...
            if (globalSettings.isPaused) {
                ...
                if (lastPausedAt && lastPausedAt > 0) {
                    (globalSettings as any).totalPausedMs += Math.max(
                        0,
                        now - lastPausedAt,
                    );
                }
    ...
    app.get("/api/skills", (req: Request, res: Response) => {
        ...
        for (const [uid, user] of Object.entries(userData)) {
            if (
                ((user as any).total_damage &&
                    (user as any).total_damage.total > 0) ||
                (user as any).taken_damage > 0 ||
                ((user as any).total_healing &&
                    (user as any).total_healing.total > 0)
            ) {
    ...
    app.post("/api/manual-group", async (req: Request, res: Response) => {
        if (!(globalSettings as any).manualGroup) {
            (globalSettings as any).manualGroup = {
                enabled: false,
                members: [],
            };
        }
    ...
    app.post(
        ...
        async (req: Request, res: Response) => {
            ...
            ).manualGroup.members.filter((m: any) => m.uid !== uid);
            await fsPromises.writeFile(
                SETTINGS_PATH,
                JSON.stringify(globalSettings, null, 4),
                "utf8",
        ...
        async (req: Request, res: Response) => {
            ...
            for (const [uid, player] of Object.entries(userData)) {
                if (
                    ...
                    if (
                        (playerRegistry as any)[uidStr] &&
                        (playerRegistry as any)[uidStr].name !==
                            (player as any).name
                    ) {
                        console.log(
        ...
        try {
            ...
            if (!inBossEncounter) {
                ...
                if (bossWithCombat) {
                    ...
                    activeBossName =
                        bossWithCombat[1]?.name || String(activeBossId);
                    lastBossSeenAt = now;
                    (async () => {
                        try {
                            ...
                            console.log(
                                `Boss ${activeBossId} (${activeBossName}) detected. Statistics reset for new encounter (players preserved).`,
                            );
                        } catch (e) {
                            console.error(
                                "Failed to reset statistics on boss start:",
                                e,
                            );
                        }
                    })();
                ...
                const bossStillSeen = entries.some(
                    ([id]) => parseInt(id, 10) === activeBossId,
                );
                if (bossStillSeen) {
                    lastBossSeenAt = now;
                } else if (
                    lastBossSeenAt &&
                    now - lastBossSeenAt > BOSS_LOST_GRACE_MS
                ) {
